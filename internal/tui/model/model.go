...
type model struct {
	...
	grepRegex        *regexp.Regexp // compiled regex from grepQuery; nil when query is empty
	...
	grepMatchFileSet map[int]bool // set of rawVisibleFiles indices that have at least one grep match; nil = no active filter
